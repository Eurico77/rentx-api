...
class RefresTokenUseCase {
  ...
  async execute(token: string): Promise<ITokenResponse> {
    ...
    const userToken = await this.usersTokenRepository.findByUserIdAndRefreshToken(sub, token);
    ...
    const expiresDate = this.dateProvider.addDays(auth.expires_in_refresh_token_days);
