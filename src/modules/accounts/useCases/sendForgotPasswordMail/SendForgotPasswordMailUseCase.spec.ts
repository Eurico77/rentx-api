...
describe('Send Forgot Mail', () => {
  ...
  it('should not be able to send an email if user does not exists', async () => {
    await expect(sendForgotPasswordMailUseCase.execute('nonexistent@mail.com')).rejects.toEqual(new AppError('User does not exists'));
  });

  it('should be able to create a new user token', async () => {
    const createUserTokenSpy = jest.spyOn(usersTokensRepositoryInMemory, 'create');
